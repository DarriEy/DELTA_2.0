...
async def process_input(
    ...
    llm_response, error = await get_chat_service().process_user_input(db, user_input, conversation_id, background_tasks)
    if error:
...
async def process_input_stream(
    ...
    return StreamingResponse(
        get_chat_service().process_user_input_stream(db, user_input, conversation_id),
...
async def generate_image_route(prompt_data: ImagePrompt):
    try:
        data_uri = await get_media_service().generate_image_data_uri(prompt_data.prompt)
...
async def text_to_speech(request: Request):
    try:
        ...
        text = data.get("text")
        payload = get_media_service().synthesize_speech(text)
...
async def get_summary(conversation_id: int, db: Session = Depends(get_db)):
    summary = await get_chat_service().get_conversation_summary(db, conversation_id)
    if not summary:
